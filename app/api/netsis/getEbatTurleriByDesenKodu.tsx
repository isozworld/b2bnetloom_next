...
export async function getEbatTurleriByDesenKodu(DesenKodu: string): Promise<{ EBATTURU: string,DesenKodu: string}[]> {
  try {
...
console.log("Tür Data ",data);
    if (data.data.IsSuccessful && Array.isArray(data.data.Data)) {
      return data.data.Data.map((item: { EBATTURU: string,DesenKodu: string }) => item);
    } else {
